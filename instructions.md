...
This scraper extracts job listings from YONDER SRL careers page (Romania only) and imports them to peviitor.ro.

Target: https://tss-yonder.com/job
...
1. **Start with brand** - We know the brand (e.g., "Yonder")
...
8. **Scrape new jobs** - Extract jobs from Yonder careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts YONDER jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Yonder careers page (Romania)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape YONDER (HTML parsing)
    │
    ▼
transformJobsForSOLR()
    ├── Filter: keep only Romanian locations
    │         (Cluj-Napoca, Iasi, etc)
    ├── Fallback: "România" for unknown
    └── Format: proper title case, uppercase company
    │
    ▼
